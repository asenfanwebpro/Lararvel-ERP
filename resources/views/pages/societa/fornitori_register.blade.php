    ...
    <div class="row">
        ...
        <div class="col">
            <ul class="nav nav-pills flex-column mt-md-0 mt-1">
                ...
                <li class="nav-item"><a class="nav-link d-flex py-75 btn-warning" href="{{route('fornitori.view')}}" ><i class="fa fa-list mr-50 font-medium-3"></i>Torna alla lista</a></li>
            </ul>
        </div>
        <!-- left menu section -->
        
        <!-- right content section -->
        {{ Form::open(array('route' => 'fornitori.save','method'=>'post','id'=>'ss_form', 'files'=>'true')) }}
        <div class="col">
            <div class="card">
                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="vertical-info" role="tabpanel" aria-labelledby="vertical-info" aria-expanded="false">
                                    ...
                                    <div class="form-group">
                                        <select id="categorylist" name="categorylist[]" class="select2 form-control" multiple="multiple" >
                                            @if(isset($categories) && count($categories) > 0)
                                            @foreach($categories as $category)
                                            <?php $categorylist = explode(',',$data->category); ?>                                            
                                            <option value="{{$category->id}}" <?php echo (array_search($category->id, $categorylist) !== false)?"selected":"" ?> >{{$category->category}}</option>
                                            @endforeach
                                            @endif
                                        </select>
                                    </div>
