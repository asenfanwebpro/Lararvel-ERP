        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    @if(strpos(Session()->get('role'), 'societalist_create') > 0)
                    <a style="float:right" href="{{route('societa.register')}}" class="btn btn-warning mr-1 mb-1"><i class="feather icon-star"></i> Nuovo Societa</a>
                    @endif
                </div>
                                   ...
                                   @if(isset($data) && !empty($data))
                                     @foreach($data as $value)                     
                                        <tr id="row{{$value->id}}">
                                            ...
                                            <td>{{$value->fax}}</td>
                                            <td>
                                                @if(strpos(Session()->get('role'), 'societalist_edit') > 0)
                                                <a href="{{route('societa.edit', $value->id)}}"><i class="fa fa-pencil fa-2x" ></i></a> &nbsp;
                                                @endif
                                                @if(strpos(Session()->get('role'), 'societalist_delete') > 0)
